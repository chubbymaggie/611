...
class instruction:
	...
	def action(self, num, counter):
		...
		if(self.ifstage == False):
			...
			if((instruction.mainif_busy == False) and ((num == 0) or (obj_list[num - 1].structflag == 0 and obj_list[num - 1].idstage == True))):
				...
				if (instruction.maindatabus_busy == False) or (instruction.maindatabus_busy == True and instruction.databussetter == num):
					...
					if self.instructionloadcounter == 0:
						...
						print "IF Complete , Bus set false"		
		...
		#"""WRITING THE ID STAGE FUNCTIONALITY HERE"""
			...
			if((instruction.mainid_busy == False) or (instruction.mainid_busy == True and instruction.idsetter == num)):
				...
				if flagraw == 1 or flagwaw == 1:
					...
					if self.code[0] in ["div.d"]:
						if instruction.maindiv_busy == True and instruction.maindiv_pipe == False:
							self.structflag = 1
							print self.code,  
							print "stalled in ID"
					elif self.code[0] in ["mul.d"] and instruction.mainmul_pipe == False:		
						if instruction.mainmul_busy == True:
							self.structflag = 1
							print self.code,  
							print "stalled in ID"
					elif self.code[0] in ["add.d", "sub.d"] and instruction.main_add_pipe == False:
						if instruction.mainadd_busy == True:
							self.structflag = 1
							print self.code,  
							print "stalled in ID"
					elif self.code[0] in ["l.d", "lw", "sw", "s.d"] and instruction.stuckinui == True:
						self.structflag = 1		
					if self.structflag == 0:		 			
						self.setidfalse = 1
						self.idstage = True
					...
					if self.code[0] == "j":
						...
						if jumper > self.line_of_code and jumper != None:
							...
							if self.line_of_code + 1 in [x[0] for x in instruction.instruction_cache]:
								instruction.instructioncachemaincount += 1
								instruction.instructioncachehitcount += 1
							elif self.line_of_code + 1 not in [x[0] for x in instruction.instruction_cache]:	
								instruction.instructioncachemaincount += 1
							instruction.ifsetter = num + 1
							obj_list[num + 1].setiffalse = 1
							for hider in range(num, len(obj_list)):
								obj_list[hider].temphideme = 1
		...
		#Writing the code for the exec stage
			...
			if self.code[0] == "lw":
				if self.uidone == False and instruction.mainui_busy == False:
					...
					if instruction.mem == False:
						instruction.stuckinui = False
					#print "Entered exec"
					...
					if self.lwstageone == False:
						...
						if (instruction.maindatabus_busy == False  and instruction.instructionbusrequest == False) or (instruction.maindatabus_busy == True and instruction.databussetter == num and instruction.instructionbusrequest == False ):
							instruction.maindatabus_busy = True
							#print "Bus taken"
							instruction.databussetter = num
							...
							if self.lwmemlatency == 0:
								...
								self.memdone = True
								self.setmemfalse = 1
						elif instruction.instructionbusrequest == True:
							self.setdatabusfalse = 1
							if obj_list[instruction.databussetter].setdatabusfalse:
			...
			if self.code[0] == "sw":
					...
					if self.ldstageone == False:
						...
						if (instruction.maindatabus_busy == False  and instruction.instructionbusrequest == False) or (instruction.maindatabus_busy == True and instruction.databussetter == num and instruction.instructionbusrequest == False):
							...
							if self.ldtwomemlatency == 0:
								...
								self.setmemfalse = 1
								self.memdone = True
						elif instrction.instructionbusrequest == True:
							self.setdatabusfalse = 1
							if obj_list[instruction.databussetter].setdatabusfalse:
								instruction.maindatabus_busy = False
								self.ldtwomemlatency += 1		
				...
				#STRUCTURAL HAZARD THINGY	
					...
					self.hazards[3] = "Y"	
				elif((self.uidone == True and instruction.mem == False and self.memdone == False )):
					instruction.stuckinui = False
					instruction.memsetter = num
					instruction.mem = True
					self.execstage = True
			...
			priorflag = 0
			word = self.code[0]
			for numerals in range(num + 1, len(obj_list)):
				if obj_list[numerals].execstage == True and obj_list[numerals].wbstage == False:
					if instruction.writebackdict[obj_list[numerals].code[0]] > instruction.writebackdict[self.code[0]]:
						print "Prior flag is set"
						priorflag = 1
						break

			if self.wbstage == False and instruction.mainwb_busy == False and priorflag == 0:
...
def main():
	...
	config =filter(lambda k: k != "", [x.lower().strip() for x in (open(sys.argv[4], r"U")).readlines()])
	latencies = []
	pipelined = []	
	#print config
	for i in config:
	...
	#print pipelined
	for i in latencies:
		if i[0].isdigit() and (i[1] == "yes" or i[1] == "no"):
			pass
		else:
			print "Error"	
...
#<---------------SETTING GLOBALS FOR THE FIRST TIME IN THE INSTRUCTION CLASS--------------------------------------------------------------------------->
	...
	if instruction.main_add_latency <= 0:
		print "latency needs to be postive"
		sys.exit(1)
	instruction.mainmul_latency = int(config.fpmult)
	if instruction.mainmul_latency <= 0:
		print "latency needs to be postive"
		sys.exit(1)
	instruction.maindiv_latency = int(config.fpdiv)
	if instruction.maindiv_latency <= 0:
		print "latency needs to be postive"
		sys.exit(1)
	instruction.mainmemory_latency = int(config.mem)
	if instruction.mainmemory_latency <= 0:
		print "latency needs to be postive"
		sys.exit(1)
	instruction.icache_latency = int(config.icac)
	if instruction.icache_latency <= 0:
		print "latency needs to be postive"
		sys.exit(1)
	instruction.dcache_latency = int(config.dcac)
	if instruction.dcache_latency <= 0:
		print "latency needs to be postive"
		sys.exit(1)
	...
	list_of_firsts = [x[0] for x in clean_code]
	#print clean_code

	if "bne" in list_of_firsts or "beq" in list_of_firsts or "j" in list_of_firsts:
		if len(labels) == 0:
			print "Error. Invalid branch instruction without a label"
			sys.exit(1)

	for i in clean_code:
		if i[0] in ["l.d", "lw", "s.d", "sw"]:
			if len(i) != 3:
				print "Unexpected Address Format"
				print "Load length"
				sys.exit(1)
		elif i[0] in ["mul.d", "div.d", "add.d", "sub.d", "dadd", "daddi", "dsub", "dsubi", "and", "andi", "or", "ori", "beq", "bne"]:
			if len(i) != 4:
				print "Unexpected Address format"
				"print add length"
				sys.exit(1)
	...
	for i in clean_code:
		if i[0] in ["mul.d", "div.d", "add.d", "sub.d"]:
			if i[1] not in ["f" + str(h) for h in range(0,32)]:
				print "Unexpected Address Format"
				print "mul.d type reg 1"
				sys.exit(1)						
			if i[2] not in ["f" + str(h) for h in range(0,32)]:
				print "Unexpected Address Format"
				print "mul.d type reg 2" 
				sys.exit(1)
			if i[3] not in ["f" + str(h) for h in range(0,32)]:
				print "Unexpected Address Format"
				print "mul.d type reg 3"
				sys.exit(1)	


		elif i[0] in ["and", "or", "dadd", "dsub"]:
			if i[1] not in ["r" + str(h) for h in range(0,32)]:
				print "Unexpected Address Format"
				print "dadd or and dsub type reg 1"
				sys.exit(1)						
			if i[2] not in ["r" + str(h) for h in range(0,32)]:
				print "Unexpected Address Format"
				print "dadd or and dsub type reg 2"
				sys.exit(1)
			if i[3] not in ["r" + str(h) for h in range(0,32)]:
				print "Unexpected Address Format"
				print "dadd or and dsub type reg 3"
				sys.exit(1)	


		elif i[0] in ["andi", "ori", "daddi", "dsubi"]:
			if i[1] not in ["r" + str(h) for h in range(0,32)]:
				print "Unexpected Address Format"
				print "Immediate reg 1"
				sys.exit(1)						
			if i[2] not in ["r" + str(h) for h in range(0,32)]:
				print "Unexpected Address Format"
				print "Immediate reg 2"
				sys.exit(1)
			if i[3].isdigit() != True:
				print "Unexpected Address Format"
				print "Immediate reg 3"
				sys.exit(1)
			...
			if i[3] not in labels:
				print "Unexpected Address Format"
				print "lbael for branch"
				sys.exit(1)

		elif i[0] in ["j"]:
			if i[1] not in labels:
				print "Unexpected Address Format"
				print "label for jump"
				sys.exit(1)

		elif i[0] in ["l.d", "s.d"]:
			if i[1] not in ["f" + str(h) for h in range(0,32)]:	
				print "Unexpected Address Format"
				print "load / store doubel reg 1"
				sys.exit(1)
			match = re.search("\d+\(r\d+\)", i[2])	
			if not match:
				print "Unexpected Address format"
				print "load / store doubel reg 2"
				sys.exit(1)

		elif i[0] in ["lw", "sw"]:
			if i[1] not in ["r" + str(h) for h in range(0,32)]:	
				print "Unexpected Address Format"
				print "load / store word reg 1"
				sys.exit(1)
			match = re.search("\d+\(r\d+\)", i[2])
			if not match:
				print "Unexpected Address format"
				print "load / store word reg 2"
				sys.exit(1)		 				


	#%%%% MAKING WRITE BACK_PRIORITIES %%%%#
	instruction.writebackdict = {}
	if instruction.maindiv_pipe == True:
		instruction.writebackdict["div.d"] = instruction.maindiv_latency
	elif instruction.maindiv_pipe == False:
		instruction.writebackdict["div.d"] = 100 + instruction.maindiv_latency

	if instruction.mainmul_pipe == True:
		instruction.writebackdict["mul.d"] = instruction.mainmul_latency
	elif instruction.maindiv_pipe == False:
		instruction.writebackdict["mul.d"] = 100 + instruction.mainmul_latency
	if instruction.main_add_pipe == True:
		instruction.writebackdict["add.d"] = instruction.main_add_latency
		instruction.writebackdict["sub.d"] = instruction.main_add_latency
	elif instruction.main_add_pipe == False:
		instruction.writebackdict["add.d"] = 100 + instruction.main_add_latency
		instruction.writebackdict["sub.d"] = 100 + instruction.main_add_latency

	for i in ["lw", "sw", "l.d", "s.d", "dadd", "daddi", "or", "ori", "dsub", "dsubi", "and", "andi"]:
		instruction.writebackdict[i] = 2	
							


	print instruction.writebackdict	
	...
	while(counter < 500):
		...
		for num in range(0, len(obj_list)):
			if obj_list[num].hideme == 0 and obj_list[num].temphideme == 0:
				obj_list[num].action(num, counter)
		#IF STAGE
		for hidersetter in range(0, len(obj_list)):
			obj_list[hidersetter].temphideme = 0
		...
		#print counter, instruction.stuckinui	
		counter = counter + 1
		#print counter
		...
		#print "---------------------------------------------------"
		#print counter	
		#print instruction.maindatabus_busy
		#print instruction.databussetter
		#print "---------------------------------------------------"
		#if counter == 50:
		#	print instruction.instructionbusrequest
	...
	print "************************************************"
	print "Number of Data Cache Access : " + str(instruction.datacachemaincount)
	print "Number of Data Cache Hits : " + str(instruction.datacachehitcount)
	print "************************************************"
	for i in obj_list:
		print i.result
	print "***********************************"
