...
#IS ACTUALLY IN ICACHE, IF SO WRITE, IF NOT MOVE ON.
#DATA HAS TO BE CONVERTED INTO A DICTIONARY FILE THAT HAS ADDRESSING FROM 256 Onwards.
obj_list = []
class configuration:
		...
		#Writing the code for the exec stage
			...
			if self.code[0] == "lw":
				if self.uidone == False and instruction.mainui_busy == False:
					...
					self.hazards[3] = "Y"	
				elif((self.uidone == True and instruction.maindatabus_busy == False and self.memdone == False and instruction.instructionbusrequest == False) or (self.uidone == True and instruction.maindatabus_busy == True 
					...
					if self.lwcounter == 0:
						...
						if "(" in self.code[2]:
							...
							answer = instruction.memory_data[number]
				...
				#STRUCTURAL HAZARD THINGY	
					...
					self.hazards[3] = "Y"	
				elif((self.uidone == True and instruction.maindatabus_busy == False and self.memdone == False and instruction.instructionbusrequest == False)):
					instruction.databussetter = num
					instruction.maindatabus_busy = True
					self.execstage = True
					self.setdatabusfalse = 1
...
#<---------------SETTING GLOBALS FOR THE FIRST TIME IN THE INSTRUCTION CLASS--------------------------------------------------------------------------->
	...
	for i in obj_list:
		print i.result
	#print instruction.memory_data
	#print len(instruction.memory_data)	
