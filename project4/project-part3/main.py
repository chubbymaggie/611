...
class instruction:
	...
	def __init__(self, code, raw_list, war_list, waw_list, dest, line_of_code):
		...
		self.hazards = [None, None, None, None]
		self.ifstage = False
		self.setiffalse = 0
		self.idstage = False
	...
	def action(self, num, counter):
		...
		#"""WRITING THE ID STAGE FUNCTIONALITY HERE"""
			...
			if((instruction.mainid_busy == False) or (instruction.mainid_busy == True and idsetter == num)):
...
#<---------------SETTING GLOBALS FOR THE FIRST TIME IN THE INSTRUCTION CLASS--------------------------------------------------------------------------->
	...
	instruction.ref_objlist = obj_list
	#print obj_list
	#print "----------------------"
	#print instruction.ref_objlist
	counter = 1
	while(obj_list[0].result[0] == None):
