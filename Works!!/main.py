...
class instruction:
	...
	def action(self, num, counter):
		...
		if(self.ifstage == False):
			...
			elif((instruction.mainif_busy == True) and (instruction.ifsetter == num) and self.ifcache_miss == True):
				if (instruction.maindatabus_busy == False) or (instruction.maindatabus_busy == True and instruction.databussetter == num) or (instruction.maindatabus_busy == True and obj_list[instruction.databussetter].buscount == 2 and instruction.databussetter != num):
					
					if obj_list[instruction.databussetter].buscount == 2:
						if obj_list[instruction.databussetter].code[0] in ["lw", "sw"]:
							obj_list[instruction.databussetter].lwmemlatency += 2
						elif obj_list[instruction.databussetter].code[0] in ["l.d", "s.d"]:
							if obj_list[instruction.databussetter].dude == 2:
								obj_list[instruction.databussetter].ldonememlatency += 2
							elif obj_list[instruction.databussetter].dude == 4:
								obj_list[instruction.databussetter].ldtwomemlatency += 2	 		
					self.instructionloadcounter = self.instructionloadcounter - 1
				...
				if self.idstage == True:
					if self.code[0] == "j":
						...
						if jumper > self.line_of_code and jumper != None:
							...
							self.execstage = True
							self.wbstage = True
							obj_list[num + 1].idstage = True
							obj_list[num + 1].execstage = True
							obj_list[num + 1].wbstage = True
							for stuff in range(num  + 2 , num + diff):
							...
							obj_list[num + 1].execstage = True
							obj_list[num + 1].wbstage = True
							for stuff in range(num + 2, len(obj_list)):
								obj_list[stuff].hideme = 1
								obj_list[stuff].ifstage = True
								obj_list[stuff].idstage = True
								obj_list[stuff].execstage = True
...
#<---------------SETTING GLOBALS FOR THE FIRST TIME IN THE INSTRUCTION CLASS--------------------------------------------------------------------------->
	...
	while(counter < 10000):
		...
		for stuff in range(0, len(obj_list)):
			obj_list[stuff].temphideme = 0	
		counter = counter + 1
